...
{ 	
  ...
  GPIO_ResetBits(GPIOB,GPIO_Pin_5|GPIO_Pin_12);
...
void NRF24L01_RX_Mode(u8 rate)
{
  	...
  	NRF24L01_Write_Reg(NRF_WRITE_REG+RF_CH,rate);	     //设置RF通信频率		  
...
{														 
  	...
  	NRF24L01_Write_Reg(NRF_WRITE_REG+RF_CH,rate);       //设置RF通道为40
  	NRF24L01_Write_Reg(NRF_WRITE_REG+RF_SETUP,0x0f);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
